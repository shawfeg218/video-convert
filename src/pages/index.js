	...
	return (
		<main className="flex min-h-screen flex-col items-center justify-between ">
			<div className="flex flex-wrap min-h-screen w-full justify-around px-8 py-10 ">
				<section className="min-h-full w-full max-w-2xl bg-white px-2 pt-3 pb-1 rounded-md">
					...
					<ul className="max-h-96 overflow-y-auto mt-2 h-3/4">
						{files.length === 0 && (
							<p className="text-center mt-12">No files in this folder.</p>
						)}
						{files.map((file) => (
							<div
								key={file}
					...
					<div className="mt-2 border-t-2 max-h-28 overflow-y-scroll ">
						{waiting.length === 0 ? (
							...
							<>
								<div className="flex">
									<p>
										{waiting[0].file} to {waiting[0].format.toUpperCase()}{' '}
										轉檔中...
									</p>
					...
					{message && (
						<div className="flex justify-center absolute top-1/2 left-0 w-full">
							<div className="border-2 border-slate-300 p-2 rounded-md bg-white max-w-md">
								<p className="font-bold text-xl text-center overflow-hidden">
									{message}
								</p>
							</div>
						</div>
